...
public class JimpleTextDocumentService extends MagpieTextDocumentService {
  private final Map<String, SignaturePositionResolver> docSignaturePositionResolver = new HashMap<>();
  ...
  public void didOpen(DidOpenTextDocumentParams params) {
    ...
    docSignaturePositionResolver.put(params.getTextDocument().getUri(), new SignaturePositionResolver(params.getTextDocument().getUri(), params.getTextDocument().getText()));
  ...
  public CompletableFuture<Either<List<? extends Location>, List<? extends LocationLink>>> definition(TextDocumentPositionParams position) {
    ...
    return getServer().pool(() -> {
      final Signature sig = docSignaturePositionResolver.get(position.getTextDocument().getUri()).resolve(position.getPosition());
      if (sig == null) {
...
//        parser.file().enterRule(signatureOccurenceAggregator);
  ...
  public CompletableFuture<List<? extends Location>> references(ReferenceParams params) {
    ...
    return getServer().pool(() -> {
      ...
      final SignaturePositionResolver resolver = docSignaturePositionResolver.get(params.getTextDocument().getUri());
      final Signature sig = resolver.resolve(params.getPosition());
      if(sig == null ){
        return null;
      }

      boolean includeDef = params.getContext() != null && params.getContext().isIncludeDeclaration();
      if( sig instanceof ClassType){
        Optional<? extends AbstractClass<? extends AbstractClassSource>> aClass = getServer().getView().getClass(((ClassType) sig));
        if(aClass.isPresent()){
          SootClass sc = (SootClass) aClass.get();
          list.add(new Location(Util.classToUri(sc), Util.positionToRange(sc.getPosition())));
        }
      }else if( sig instanceof MethodSignature){
        Optional<? extends AbstractClass<? extends AbstractClassSource>> aClass = getServer().getView().getClass(((MethodSignature) sig).getDeclClassType());
        if(aClass.isPresent()){
          SootClass sc = (SootClass) aClass.get();
          final Optional<? extends Method> method = sc.getMethod(((MethodSignature) sig));
          method.ifPresent(value -> list.add(new Location(Util.classToUri(sc), Util.positionToRange(((SootMethod) value).getPosition()))));
        }
      }else if( includeDef ) {
        if (sig instanceof FieldSignature) {
          Optional<? extends AbstractClass<? extends AbstractClassSource>> aClass = getServer().getView().getClass(((FieldSignature) sig).getDeclClassType());
          if (aClass.isPresent()) {
            SootClass sc = (SootClass) aClass.get();
            final Optional<? extends Field> field = sc.getField(((FieldSignature) sig));
            field.ifPresent(value -> list.add(new Location(Util.classToUri(sc), Util.positionToRange(((SootField) value).getPosition()))));
          }
        }
      }

      final Collection<SootClass> classes = (Collection<SootClass>) getServer().getView().getClasses();
      if(sig instanceof ClassType) {
            ...
            for (Stmt stmt : body.getStmtGraph().nodes()) {
              for (Value usesAndDef : stmt.getUsesAndDefs()) {
                if ( usesAndDef.getType() == sig ) {
                  list.add(new Location(Util.classToUri(sc), Util.positionToRange(stmt.getPositionInfo().getStmtPosition())));
                }
              }
            }
          }
        ...
        for (SootClass sc : classes) {
          for (SootMethod method : sc.getMethods()) {
            ...
            for (Stmt stmt : body.getStmtGraph().nodes()) {
              for (Value usesAndDef : stmt.getUsesAndDefs()) {
                if (usesAndDef instanceof JFieldRef && ((JFieldRef) usesAndDef).getFieldSignature().equals(sig)) {
                  list.add(new Location(Util.classToUri(sc), Util.positionToRange(stmt.getPositionInfo().getStmtPosition())));
                }
              }
            }
        ...
        for (SootClass sc : classes) {
          for (SootMethod method : sc.getMethods()) {
            final Body body = method.getBody();
            for (Stmt stmt : body.getStmtGraph().nodes()) {
              if(stmt instanceof JInvokeStmt || ( stmt instanceof JAssignStmt && ((JAssignStmt) stmt).getRightOp() instanceof AbstractInvokeExpr) ) {
                list.add(new Location(Util.classToUri(sc), Util.positionToRange(stmt.getPositionInfo().getStmtPosition())));
              }
            }
          }
